...
class AISTMA_Standalone_Editor {
    ...
    public function handle_check_enhancement_eligibility() {
        ...
        // A post is eligible if it was generated by this plugin.
        // 0 or empty enhancements_limit means unlimited — treat as eligible.
        // Covers both master API posts (have package_id) and personal API posts (have request_id only).
        $enhancements_limit = get_post_meta( $post_id, 'ai_story_maker_enhancements_limit', true );
        $package_id         = get_post_meta( $post_id, 'ai_story_maker_package_id', true );
        $request_id         = get_post_meta( $post_id, 'ai_story_maker_request_id', true );

        $eligible = ! empty( $package_id ) || ! empty( $request_id );
