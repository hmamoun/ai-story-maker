...
class AISTMA_Admin {
	...
	public function aistma_admin_enqueue_scripts( $hook_suffix ) {
		// Only load wizard and rating scripts on relevant pages
		$load_wizard = false;
		$load_rating = false;
		...
		// Always enqueue admin script (needed for social media nonce across all pages)
		wp_enqueue_script(
			'aistma-admin-js',
		...
		// Enqueue wizard scripts and styles (only on relevant pages)
		if ( $load_wizard ) {
			wp_enqueue_script(
				'aistma-wizard-js',
			...
			wp_localize_script( 'aistma-wizard-js', 'aistmaWizardL10n', array(
				...
				'siteContext'       => esc_js( AISTMA_Activation_Wizard::build_site_context() ),
		...
		// Enqueue rating modal CSS and JS (only on posts page)
		if ( $load_rating ) {
			wp_enqueue_style(
				'aistma-rating-css',
			...
			// Localize rating script with nonces (only when script is enqueued)
